...
# AI Risk Schemas - UPDATED TO MATCH NEW JSON STRUCTURE
class RiskAssessment(BaseModel):
    risk_score: float
    ml_risk_label: RiskLevel
    ml_confidence: float
...
class Recommendation(BaseModel):
    recommended_pqc: str
    estimated_effort_days: int
    migration_complexity: Optional[str] = None  # Made optional

class Migration(BaseModel):
    timeline: str

class Explainability(BaseModel):
    top_risk_factors: Optional[List[str]] = []  # Made optional with default
    recommendation_rationale: Optional[str] = ""  # Made optional with default
...
class CVEDetails(BaseModel):
    """Optional CVE details for vulnerabilities"""
...
class AIRiskRecord(BaseModel):
    ...
    risk_assessment: RiskAssessment
    recommendation: Recommendation
    migration: Optional[Migration] = None  # Made optional
    explainability: Optional[Explainability] = None  # Made optional
    cve_details: Optional[CVEDetails] = None  # Added for CVE records
    
    class Config:
        # This allows the model to accept extra fields without errors
        extra = "ignore"
